<div>
    <div class="px-5 mb-4 h-max">
        ...
        {{-- Tambah Peringatan --}}
        <a href="{{ route('peringatan.create') }}" type="button"
        ...
        <div class="relative overflow-x-auto shadow-md rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 sm:rounded-lg">
                ...
                <tbody>
                    @foreach ($this->peringatans as $peringatan)
                        <tr wire:key="{{ $peringatan->id }}"
                            ...
                            <td class="px-6 py-4">
                                ...
                                <div class="italic font-normal">
                                    {{ \Carbon\carbon::parse($peringatan->tgl_klarifikasi)->format('d-m-Y') }}
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-base font-semibold">
                            ...
                            <td class="px-6 py-4">
                                {{ \Carbon\carbon::parse($peringatan->batas_setor)->format('d-m-Y') }}
                            </td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-2 py-1 font-semibold leading-tight
                                        {{ $peringatan->is_lunas == 1 ? 'text-green-700 bg-green-100 rounded-full dark:text-green-50 dark:bg-green-600' : 'text-orange-700 bg-orange-100 rounded-full dark:text-orange-100 dark:bg-orange-600' }}">
                                    {{ $peringatan->is_lunas == 1 ? 'Success' : 'On Process' }}
                                </span>
                            </td>
                            <td class="px-6
                                    py-4 text-center">
                                <a type="button" href="peringatan/{{ $peringatan->id }}"
                                ...
                                <a type="button" href="peringatan/{{ $peringatan->id }}/edit"
                                ...
                                <button type="button" wire:click="deletePeringatan({{ $peringatan->id }})"
                                    wire:confirm="Apakah Anda yakin ingin menghapus Peringatan ini?"
