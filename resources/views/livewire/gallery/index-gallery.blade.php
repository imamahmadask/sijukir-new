                ...
                <tbody>
                    @foreach ($this->galleries() as $gallery)
                        <tr wire:key="{{ $gallery->id }}"
                            ...
                            <td class="px-6 py-4">
                                {{ Str::limit($gallery->deskripsi, 40) }}
                            </td>
                            <td class="px-6 py-4">
                                {{ $gallery->kategori }}
                            </td>
                            <td class="px-6 py-4">
                                {{ date('d-M-Y', strtotime($gallery->tanggal)) }}
                            </td>
                            <td class="px-6 py-4">
                                @if ($gallery->gambar)
                                    <div class="flex -space-x-4 rtl:space-x-reverse">
                                        @foreach (array_slice($gallery->gambar, 0, 4) as $gambar)
                                            <img class="w-12 h-12 border-2 border-white rounded-full dark:border-gray-800"
                                                src="/storage/{{ $gambar }}" alt="">
                                        @endforeach
                                    </div>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                {{-- Modal --}}
                                <button wire:click="$dispatch('show-gallery', { id: {{ $gallery->id }} })"
                                    data-modal-target="show-gallery-modal" data-modal-toggle="show-gallery-modal"
                                    class="text-white justify-end bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-full text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">
                                ...
                                @teleport('body')
                                    ...
                                    @livewire('gallery.show-gallery')
                                @endteleport

                                <a type="button" href="gallery/{{ $gallery->id }}/edit"
                                ...
                                <button type="button" wire:click="deleteGallery({{ $gallery->id }})"
